...
- **Structured data** — `src/components/seo/organization-jsonld.tsx` emits an
  `NGO` JSON-LD block in every page's `<head>`, carrying the trust's name,
  logo, phone, email and district. Each locale renders its own name and
  description, with the other three scripts as `alternateName`, so a search in
  Telugu and a search in English resolve to the same organisation. It states
  only confirmed facts — deliberately nothing from `placeholder-data.ts`, since
  those impact figures are invented and must not be published as machine-
  readable claims. Check changes against the
